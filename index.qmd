...
pacman::p_load(tidyverse, kableExtra, here, ggspatial, cowplot)
...
latest_db_file <- list.files(
    path = db_directory,
    # This pattern finds all files that start with "Project_ArHa_database_" and end with ".rds"
    pattern = "^Project_ArHa_database_.*\\.rds$",
    full.names = TRUE # This is crucial to get the full path for reading
  ) |>
  sort(decreasing = TRUE) |> # Sort descending, newest file will be first
  head(1) # Take the first file from the list

# Add a check to ensure a file was actually found
if (length(latest_db_file) == 0) {
  stop("No database file found in the directory: ", db_directory)
}

# Load the most recent database file
...
# This pipe filters for Lassa, joins the required data, and finds the earliest
# record for each host/assay combination within the endemic zone.
